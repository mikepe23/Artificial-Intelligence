...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            self.beliefs[ghostPosition] = self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPosition, self.getJailPosition()) * self.beliefs[ghostPosition]
    ...
    def elapseTime(self, gameState):
        ...
        # precompute distributions to reduce runtime
        d = dict()
        for p in self.allPositions:
            d[p] = self.getPositionDistribution(gameState, p)
        ...
        for p in self.allPositions:
            ...
            for oldPos in self.allPositions:
                newPosDist = d[oldPos]
                s += newPosDist[p] * beliefs_copy[oldPos]
            self.beliefs[p] = s
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            cache[pos] = self.getPositionDistribution(gameState, pos)
        for p in self.particles:
            newParticleList.append(cache[p].sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        shuffle(particles)
        self.particles = [particles[i % len(particles)] for i in range(self.numParticles)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                jointProbability *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), p[i], self.getJailPosition(i))
            weights[p] += jointProbability
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weights.sample() for _ in range(self.numParticles)]
